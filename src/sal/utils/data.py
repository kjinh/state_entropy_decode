...
def get_dataset(config: Config) -> Dataset:
    ...
    starts = config.dataset_start if config.dataset_start else 0
    ends = config.dataset_end if config.dataset_end else len(dataset)
    if config.num_samples is not None :
        ends = min(ends, starts + config.num_samples)
    import numpy as np
    ranges = np.array_split(range(starts, ends), 1)
    datasets = [dataset.select(i.tolist()) for i in ranges]
    return datasets
...
def save_dataset(dataset, idx, config):
    if config.push_to_hub:
        ...
        dataset.to_json(
            f"{config.output_dir}/{config.approach}_completions_{idx}.jsonl", lines=True
        )
        logger.info(
