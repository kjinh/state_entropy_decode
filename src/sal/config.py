...
class Config:
    ...
    # embedding_model: str = "microsoft/deberta-v3-large" # 'meta-llama/Llama-3.2-1B-Instruct'
    embedding_model: str= 'meta-llama/Llama-3.2-1B-Instruct'
    # embedding_model: str = "intfloat/e5-large-v2" # 'meta-llama/Llama-3.2-1B-Instruct'
    gpu_memory_utilization: float = (
        0.35  # vllm is allocated 0.5 of GPU memory, the PRM uses the rest
    )
    # prm_path: str = "peiyi9979/math-shepherd-mistral-7b-prm"
    # prm_path: str = "Skywork/Skywork-o1-Open-PRM-Qwen-2.5-1.5B"
    
    state_traj: Literal["trajectory", "step"] = "step" # Whether to use the rationale for the last State created or the entire trackjectory 
    ...
    regenerate_prompt: str = "This answer is incorrect for this prompt. It is incorrect at step {regenerate_check} and correct up to that point. Referring to the answer guide, clearly show how the incorrect answer has a completely different solution from the correct answer after step {regenerate_check}."
    refine_prompt: str = 'You are a great math assistant. Your task is to examine the problem and the solution, and check if the solution is incorrect. If it is, you set wrong_check=True and the variable refine_step specifies the number of steps where it is incorrect, otherwise you return wrong_check=False and refine_step=-1.\n\nThe bool type wrong_check is whether the solution is wrong or not, and the int type refine_step variable is the step at which the solution is wrong for the first time.\n\nThe problem is as follows\n\nOnly as many steps as {steps_refine} should be validated as right or wrong.\n\nProblem: {prompt}'
    ...
    # Save options:
    output_dir: str = f'./results_0313/sa_{entropy_hyper}_model_{prm_path}_state_{state_traj}_scheduler_{scheduler}_n_{n}_svd_refine'
