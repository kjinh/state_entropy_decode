...
def _sa_beam_search(batch_of_prompts, config: Config, prm: PRM, ip: dict[str, str]) -> list[Beam]:
    rms = RMS("cuda:0")
    stat_entropy = PBE(rms, config.embedding_model, "cuda:0", ip["embed"])
    history = []
    steps_refine = config.steps_refine
    ...
    for nw in tqdm(range(config.num_iterations), desc=f"Beam search iterations with sa {config.entropy_hyper}"):
        ...
        for beam, gen_result in zip(active_beams, gen_results, strict=True):
            ...
            prompts.append(beam.prompt) #prompt
            completions.append([beam.current_text]) #answer
        ...
        stat_check = torch.tensor([b.regenerate for b in active_beams])
        if (nw+1) % steps_refine == 0 :
            for b in active_beams :
                b.regenerate = refine_answer(config.refine_prompt, b.prompt, b.current_text, steps_refine)
                # print(b.regenerate)
        else :
            for b in active_beams :
                b.regenerate = -1
        ...
        if config.entropy_hyper > 0 :
            ...
            if nw > 0 and len(active_beams[0].history) > 1:
                ...
                flat_agg = (torch.tensor(flat_agg) * (stat_check <= 0) + reward_add * (stat_check > 0) * implict_reward).tolist()
...
def sa_beam_search(examples, config: Config, prm: PRM, ip: dict[str, str]):
    problems = examples["problem"]
    beam_results, history_pers = _sa_beam_search(problems, config, prm, ip)
    # print(history_pers)
    import os
    history_path = '/home/mjmps0725/state_entropy_decode/history/history_0'
