# Fix Codes
1. `sal/models/reward_models.py`: Since the output is retrieved through the vLLM server, the code for this part has been modified.  
2. `sal/server_7b, server_8b.py`: Running the PRM model locally would take too much time, so I implemented it by building a server and communicating with it.  
3. `sal/beam_search.py`: I have added a process to calculate State Entropy by defining the state. You can experiment with different methods by modifying the `state_traj` parameter in `config.py`.  
4. `sal/utils.py`: State Entropy and the dimension reduction code used in the process are added at the very end.
5. `ev.py`: This is the code that parses the output of the model and produces the results. You can measure the result by changing the `folder` variable inside the code.
